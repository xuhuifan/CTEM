...
import pyximport
pyximport.install()
from cython_function import judge_integration_relation, process_b_ij_n, gradient_g_part, z_part_2, PG_g_sum, PG_g_z, PG_g_m, PG_g_elbo, cal_ll_integration_g, cal_ll_integration_g_future
...
class CONTEC():
    ...
    def log_likelihood(self, time_period):
        ...
        integration_g = cal_ll_integration_g(eval_points, np.asarray(time_period), self.z_ij_n_mat, self.relations[:, 2], self.m_k, index_m.astype(np.intc), sigma_g_general, np.intc(self.K))

        diff_before_g_cumsum = np.cumsum(self.diff_before_len)
        s_g_1D = diff_before_g_cumsum[index_m[0]]
        t_g_1D = diff_before_g_cumsum[index_m[-1]]

        b_z_non_0 = self.b_non_0_1D[s_g_1D:t_g_1D][:, np.newaxis]*self.z_ij_n_mat[self.mutually_before_1D[s_g_1D:t_g_1D]]
    ...
    def log_likelihood_future(self, time_period):
        ...
        mu_g_eval_points, _ = self.predictive_posterior_GP_g(eval_points, ks_eval_points_g, kappa_eval_points_g)
        sigma_g_general = sigma_f(np.asarray(mu_g_eval_points).T)
        integration_g = np.asarray(cal_ll_integration_g_future(eval_points, self.relations[:, :2].astype(np.intc), np.asarray(time_period), self.z_ij_n_mat, self.relations[:, 2], self.m_k, sigma_g_general, np.intc(self.K), np.intc(self.NumE) ,np.intc(self.N)))
