import numpy as np
from utility_K_exogenous_rate import *
...
for ite in range(contec.num_iteration):
    ...
    current_cov_params_g = copy.deepcopy(contec.cov_params_g)
    ...
    if (np.mod(ite, 20) == 0)&(ite>0):
        ...
        start_time = time.time()
        # plt.plot(elbo_seq)
        # plt.savefig('elbo_'+name+'_K_exogenous_rate.pdf', edgecolor='none', format='pdf', bbox_inches='tight')
        # plt.close()

result_path = '/home/z3522716/Add_data/CONTIM_result/result_2/'
np.savez_compressed(result_path+'result_'+name+'_K_exogenous_rate_K_'+str(K)+'.npz', contec = contec, N = N, K = K, T = T, elbo_seq = elbo_seq)
