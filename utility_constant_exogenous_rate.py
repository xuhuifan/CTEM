...
def inhomogeneous_poisson_process_sample(N, T, K):
    ...
    origin_mu_f = np.random.randn(len(origin_points))
    ...
    for i in range(N):
        for j in np.arange(i+1, N).astype(int):
            ...
            for k in range(K):
                ...
                origin_mu_k = origin_mu_f
                cov_params_k = cov_params_f
                delta_val = []
...
class CONTEC():
    def __init__(self, adam_alpha, T, N, K, pis, origin_cov_g, origin_m_val, origin_points,  origin_mu_g, cov_params, num_integration_points, num_inducing_points, relations, conv_crit=1e-4):
        ...
        self.cov_params_g = [copy.deepcopy(cov_params) for _ in range(K)]
    ...
    def calculate_PG_expectation_g(self):
        ...
        term_b_ij_n_z_ij_n_slash = self.z_ij_n_mat[self.mutually_before_1D] * self.b_non_0_1D[:, np.newaxis]
    ...
    def update_b_ij_n(self):
        ...
        b_ij_n_0 = np.sum(self.z_ij_n_mat*mu_alpha_ij, axis=1)
        ...
        b_ij_n_nslash = np.sum(self.z_ij_n_mat[self.mutually_before_1D]*self.second_part, axis=1)
    ...
    def update_z_ij_n(self):
        ...
        part_1 = self.b_0_1D[:, np.newaxis]*mu_alpha_ij
        ...
        part_2 = z_part_2(self.b_non_0_1D, self.second_part, self.ji_n_slash_index.astype(np.intc), self.diff_after_len.astype(np.intc), np.intc(self.NumE), np.intc(self.K))
        #
        ...
        z_ij_n_LL = part_1 + np.asarray(part_2) - part_3
    ...
    def calculate_posterior_GP_g(self):
        ...
        [sum_z_L, sum_z_w_L] = PG_g_sum(self.z_ij_n_mat, np.intc(self.num_integration_points), \
                                                                                    np.intc(self.K), np.intc(self.NumE), self.g_int_points, \
        ...
        b_X_term1 = self.z_ij_n_mat[self.mutually_before_1D]*self.b_non_0_1D[:, np.newaxis]
    ...
    def update_m(self):
        ...
        z_b = self.z_ij_n_mat[self.mutually_before_1D] * self.b_non_0_1D[:, np.newaxis]
    ...
    def update_pi_k(self):
        ...
        for ii in range(self.N):
            ...
            self.alpha_pi_q[ii] = part_1 + self.alpha_pi_0[ii]
    ...
    def calculate_lower_bound(self):
        ...
        """
        # ll_exogenous_rate = -np.sum(np.sum(self.pi_k, axis=0)**2-np.sum(self.pi_k**2, axis=0))*self.T
        # ll_exogenous_rate += np.sum(self.z_ij_n_mat*self.b_0_1D[:, np.newaxis]*(self.log_pi_k[self.relations[:, 0].astype(int)]+self.log_pi_k[self.relations[:, 1].astype(int)]))

        LL_f1 = -self.T*np.sum((np.sum(self.pi_k, axis=0)**2-np.sum(self.pi_k**2, axis=0)))
        LL_f2 = np.sum(self.z_ij_n_mat*self.b_0_1D[:, np.newaxis]*(self.log_pi_k[self.relations[:, 0].astype(int)]+self.log_pi_k[self.relations[:, 1].astype(int)]))

        print('LL_f1 is: ', LL_f1)
        print('LL_f2 is: ', LL_f2)
        ...
        summand_for_g = self.b_non_0_1D[:, np.newaxis]*self.z_ij_n_mat[self.mutually_before_1D]*h_X_for_g -np.log(np.cosh(0.5*self.c_X_g_1D))+0.5*((self.c_X_g_1D**2))*self.w_X_g_1D
        ...
        elbo = L_g + L_m_pi + LL_g + LL_z_b + LL_f
    ...
    def calculate_hyperparam_derivative_g(self):
        ...
        for kk in range(self.K):
            ...
            for itheta in range(2):
                ...
                dg1_X = self.z_ij_n_mat[self.mutually_before_1D, kk] * self.b_non_0_1D*self.g_s[kk].dot(dkappa_X)
                ...
                dg2_X = self.z_ij_n_mat[self.mutually_before_1D, kk] * self.b_non_0_1D*dg2_X
