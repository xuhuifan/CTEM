...
import pyximport
pyximport.install()
from cython_function import judge_integration_relation, process_z_ij_n_endogenous_rate, cal_g_integration_m, cal_LL_g_K_endogenous_rate, gradient_g_part_K_endogenous_rate, PG_g_sum_endogenous_rate, cal_ll_integration_g_endogenous_function, z_part_2_K_endogenous_rate, cal_ll_integration_g_future_endogenous_function
...
class CONTEC():
    def __init__(self, adam_alpha, T, N, K, pis, origin_cov_f, origin_cov_g, origin_m_val, origin_points, origin_mu_f, origin_mu_g, cov_params, num_integration_points, num_inducing_points, relations, conv_crit=1e-4):
        ...
        self.log_m_k = np.log(np.ones(pis.shape))
        self.m_k = np.exp(self.log_m_k)
        ...
        self.b_0_1D = np.asarray([b_ii[0] for b_ii in b_ij_n_list])[:, np.newaxis].dot(np.ones((1, self.K)))/self.K
        a = [b_ii[1:] for b_ii in b_ij_n_list]
        self.b_non_0_1D = np.concatenate(a, axis=0)[:, np.newaxis].dot(np.ones((1, self.K)))/self.K
        ...
        self.alpha_m_0 = 0.01*np.ones(self.N)
        self.beta_m_0 = 0.1*np.ones(self.N)
        ...
        for ii in range(self.N):
            ...
            self.N_judge_node.append(np.concatenate((self.relations[first_judge, 1].astype(int), self.relations[second_judge, 0].astype(int))))
    ...
    def calculate_PG_expectation_f(self):
        ...
        self.c_X_f = np.sqrt(np.asarray(self.f2_X) * self.b_0_1D.T)
    ...
    def calculate_PG_expectation_g(self):
        ...
        self.c_X_g_1D = np.sqrt(self.b_non_0_1D*self.g2_X_1D)
        self.c_int_points_g = np.sqrt(self.g2_int_points)

        self.w_X_g_1D = 0.5*np.tanh(0.5*self.c_X_g_1D)/self.c_X_g_1D
        self.w_int_points_g = 0.5*np.tanh(0.5*self.c_int_points_g)/self.c_int_points_g

        self.Lambda_g_without_v = 0.5*np.exp(-0.5*np.asarray(self.g_int_points))/np.cosh(0.5*self.c_int_points_g)


    def update_b_1D(self):
        ...
        mu_alpha_m = (self.log_m_k[(self.relations[self.phi_mk_index, 0]).astype(int)]+self.log_m_k[(self.relations[self.phi_mk_index, 1]).astype(int)])

        second_part = mu_alpha_m+self.g_X_1D/2-self.g2_X_1D*self.w_X_g_1D/2-np.log(2)
        ...
        [b_0_1D, b_non_0_1D] = process_z_ij_n_endogenous_rate(first_part, second_part, np.asarray(self.diff_before_len).astype(np.intc), np.intc(self.NumE), self.before_max, np.intc(self.K))
    ...
    def calculate_posterior_GP_g(self):
        ...
        a1 = (np.exp(self.log_m_k[self.relations[:, 0].astype(int)] + self.log_m_k[self.relations[:, 1].astype(int)]))

        [inte_L, inte_w_L] = PG_g_sum_endogenous_rate(np.intc(self.num_integration_points), np.intc(self.K), np.intc(self.NumE), self.Lambda_g_without_v, a1, self.w_int_points_g, \
                                                                                    self.relations[:, 2], self.integration_points, self.T)

        A_int_points = (np.asarray(inte_w_L)).T
        B_int_points = -0.5*(np.asarray(inte_L)).T

        b_X_term1 = self.b_non_0_1D
    ...
    def update_m(self):
        self.alpha_m_q = np.zeros((self.N, self.K))
        self.beta_m_q = np.zeros((self.N, self.K))

        part_2 = np.asarray(z_part_2_K_endogenous_rate(self.b_non_0_1D, self.diff_before_len.astype(np.intc), np.intc(self.NumE), np.intc(self.K)))

        for ii in range(self.N):
            part_1 = np.sum(part_2[self.N_judge_index[ii]], axis=0)

            a1 = (np.exp(self.log_m_k[self.relations[self.N_judge_index[ii], 0].astype(int)] + self.log_m_k[self.relations[self.N_judge_index[ii], 1].astype(int)]))

            integration_m = np.asarray(cal_g_integration_m(self.Lambda_g_without_v, a1, np.intc(self.K), self.relations[self.N_judge_index[ii], 2], self.integration_points, self.T))

            d_val = np.sum(integration_m/(self.judge_per_relation[self.N_judge_index[ii]]/(self.T-self.relations[self.N_judge_index[ii], 2]))[:, np.newaxis], axis=0)

            self.alpha_m_q[ii] = d_val+part_1+self.alpha_m_0[ii]
            c_val = np.sum(self.m_k[self.N_judge_node[ii]]*((self.T-self.relations[self.N_judge_node[ii], 2])[:, np.newaxis]), axis=0)
            self.beta_m_q[ii] = c_val + self.beta_m_0[ii]

        self.log_m_k = digamma(self.alpha_m_q)-np.log(self.beta_m_q)
    ...
    def calculate_lower_bound(self):
        ...
        summand_for_f = self.b_0_1D*h_X_for_f -np.log(np.cosh(0.5*self.c_X_f.T)) +0.5*((self.c_X_f**2).T)*self.w_X_f.T
        ...
        a_val = (np.exp(self.log_m_k[self.relations[:, 0].astype(int)]+self.log_m_k[self.relations[:, 1].astype(int)]))

        h_int_points_for_g = -self.g_int_points/2-self.g2_int_points*self.w_int_points_g/2-np.log(2)


        integrand_for_g = (h_int_points_for_g-np.log(self.Lambda_g_without_v*np.cosh(self.c_int_points_g/2))  + 0.5 *(self.c_int_points_g**2)*self.w_int_points_g+1).T

        # LL_g = np.sum(np.sum(integrand_for_g, axis=1)/self.judge_per_relation*(self.T-self.relations[:,2]))-(np.sum(self.m_k)*np.sum(self.T-self.relations[:, 2]))
        # LL_g = np.sum([integrand_for_g[k].dot(self.Lambda_g_without_v.T[k])*a_val[k]/self.num_integration_points*self.T for k in range(self.K)])

        ll_g_inte = cal_LL_g_K_endogenous_rate(np.intc(self.num_integration_points), np.intc(self.NumE), self.Lambda_g_without_v, a_val, integrand_for_g, self.relations[:, 2], self.integration_points, self.T, np.intc(self.K))

        LL_g = np.sum([np.asarray(ll_g_inte[:, k])/self.judge_per_relation*(self.T-self.relations[:,2]) for k in range(self.K)])

        h_X_for_g = 0.5*np.asarray(self.g_X_1D)-0.5*np.asarray(self.g2_X_1D)*self.w_X_g_1D-np.log(2)+(self.log_m_k[self.relations[self.phi_mk_index, 0].astype(int)]+self.log_m_k[self.relations[self.phi_mk_index, 1].astype(int)])
        summand_for_g = self.b_non_0_1D*h_X_for_g -np.log(np.cosh(0.5*self.c_X_g_1D))+0.5*((self.c_X_g_1D**2))*self.w_X_g_1D

        LL_g += np.sum(summand_for_g)

        LL_z_b = -np.sum(self.b_non_0_1D*np.log(self.b_non_0_1D))-np.sum(self.b_0_1D*np.log(self.b_0_1D))
        ...
        L_m_pi += np.sum((self.alpha_m_0*np.log(self.beta_m_0))[:, np.newaxis] - gammaln(self.alpha_m_0[:, np.newaxis]) + (self.alpha_m_0[:, np.newaxis] - 1)*self.log_m_k - self.beta_m_0[:, np.newaxis]*self.m_k)
    ...
    def log_likelihood(self, time_period):
        ...
        sum_f = np.sum(self.b_0_1D[index_m]*(log_val + np.log(sigma_f((np.asarray(self.f_X).T)[index_m]))))
        ...
        integration_g = cal_ll_integration_g_endogenous_function(eval_points, np.asarray(time_period), self.relations[:, 2], self.m_k, index_m.astype(np.intc), self.relations[:, :2].astype(np.intc), sigma_g_general, np.intc(self.K))
        ...
        b_z_non_0 = self.b_non_0_1D[s_g_1D:t_g_1D]
        log_m_val = self.log_m_k[self.relations[self.phi_mk_index[s_g_1D:t_g_1D],0].astype(int)]+self.log_m_k[self.relations[self.phi_mk_index[s_g_1D:t_g_1D],1].astype(int)]
        sum_g = np.sum(b_z_non_0*(np.log(sigma_f(self.g_X_1D[s_g_1D:t_g_1D]))+log_m_val))
    ...
    def log_likelihood_future(self, time_period):
        ...
        eval_points = np.arange(time_period[0]-self.relations[-1,2], self.relations[-1,2]-self.relations[0,2]+time_period[1]-time_period[0], step = 0.001)
        ks_eval_points_g = [cov_func(self.induced_points, eval_points, self.cov_params_g[k]) for k in range(self.K)]
        kappa_eval_points_g = [self.Ks_inv_g[k].dot(ks_eval_points_g[k]) for k in range(self.K)]

        mu_g_eval_points, _ = self.predictive_posterior_GP_g(eval_points, ks_eval_points_g, kappa_eval_points_g)
        sigma_g_general = sigma_f(np.asarray(mu_g_eval_points).T)
        integration_g = np.asarray(cal_ll_integration_g_future_endogenous_function(eval_points, self.relations[:, :2].astype(np.intc), np.asarray(time_period), self.relations[:, 2], self.m_k, sigma_g_general, np.intc(self.K), np.intc(self.NumE) ,np.intc(self.N)))

        integration_val = integration_f + integration_g
    ...
    def calculate_hyperparam_derivative_f(self):
        ...
        for kk in range(self.K):
            ...
            for itheta in range(2):
                ...
                dg1_X = self.b_0_1D[:, kk]*self.f_s[kk].dot(dkappa_X)
    ...
    def calculate_hyperparam_derivative_g(self):
        ...
        for kk in range(self.K):
            ...
            for itheta in range(2):
                ...
                dg1_X = self.b_non_0_1D[:, kk]*self.g_s[kk].dot(dkappa_X)
                ...
                dg2_X = self.b_non_0_1D[:, kk]*dg2_X
                dg2_int_points = (dKtilde_int_points + 2. * np.sum(self.kappa_int_points_g[kk] * Sigma_s_mugmug.dot(dkappa_int_points),
                    axis=0))

                dL_dtheta[kk][itheta] = .5 * (np.sum(dg1_X) - np.sum(dg2_X))

                gradient_g_part_val = gradient_g_part_K_endogenous_rate(np.intc(self.num_integration_points), np.intc(self.NumE), self.Lambda_g_without_v[:, kk], a1[:, kk], self.w_int_points_g[:, kk], self.relations[:, 2], self.integration_points, self.T, -dg1_int_points,- dg2_int_points)
